...
describe('when there is initially two users in db', () => {
  ...
  test('list all users', async () => {
    ...
    expect(JSON.parse(usersAtEnd.text)).toHaveLength(usersAtStart.length)

    const usernames = JSON.parse(usersAtEnd.text).map(u => u.username)
    expect(usernames).toContain(usersAtStart[0].username)
  })
})
